...
interface Props { college: ProviderCollege; provider: string; onClose: () => void; }

export default function FileModal({ college, provider, onClose }: Props) {
  ...
  async function handleFolder() {
    ...
    try {
      await downloadBlob(
        `/admin/download-folder?provider=${encodeURIComponent(provider)}&collegeName=${encodeURIComponent(college.collegeName)}`,
        `${provider}_${college.collegeName}.zip`,
      );
    } finally { setDownloading(null); }
  }
    ...
    <div className="modalOverlay" onClick={onClose}>
      <div className="modalBox" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modalBox__footer">
          ...
          >
            <IconDownload />
            {downloading === 'folder' ? 'Zipping…' : 'Download Folder (ZIP)'}
          </button>
        </div>
      </div>
    </div>
